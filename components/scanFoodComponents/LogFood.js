...
const LogFood = ({ navigation }) => {
    ...
    if (dataFromSearch) {
      ...
      ;(async () => {
        ...
        if (foodData) {
          ...
          const newFoodData = {
            name: foodData.name,
            calories: parseFloat(foodData.nutrients?.ENERC_KCAL || 0).toFixed(
              2
            ),
          ...
          setCalories(
            parseFloat(foodData.nutrients?.ENERC_KCAL || 0).toFixed(2)
          )
  ...
  const logFood = async () => {
    ...
    try {
      ...
      Alert.alert("Success", "Food has been logged successfully!", [
  ...
  const convertNutritionalData = (unit, size) => {
    ...
    setCalories(
      parseFloat(
        (foodInfo.calories * servingSizeInGrams) / 100 + extraCalories
      ).toFixed(2)
    )
    setFats(parseFloat((foodInfo.fats * servingSizeInGrams) / 100).toFixed(2)),
      setCarbs(
        parseFloat((foodInfo.carbs * servingSizeInGrams) / 100).toFixed(2)
      ),
      setProteins(
        parseFloat((foodInfo.proteins * servingSizeInGrams) / 100).toFixed(2)
      )
  }
    ...
    <NativeBaseProvider>
      {loading ? (
        ...
        <View style={styles.container}>
          ...
          <InputAndPickerView
            title={"Serving size"}
            dropdownData={unitArray}
            size={servingSize}
            unit={servingUnit}
            changeSize={changeServingSize}
            changeUnit={changeServingUnit}
          />

          <LogInputField
            title={"Number of serving"}
            placeholder="ex 1"
            useSetNumber={true}
            changeNumber={changeNumberOfServing}
          />

          <LogInputField
            title={"Meal"}
            isDropDown={true}
            dropdownData={mealArray}
            serviceSelected={mealArray[0]}
            useMealType={changeMealType}
          />

          <LogInputField
            title={"Extra calories"}
            placeholder="ex 150"
            useExtraCals={true}
            extraCals={changeExtraCalories}
          />
          ...
          <View
            ...
            />
            <LogButtons
              isEdited={isMealEdited}
              logPress={logFood}
              mealInfoPress={getInfo}
            />
          </View>
        </View>
      )}
